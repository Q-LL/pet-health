...
截至 2026-09-12，App 已完成 P0 的记录、护理计划、提醒与轻量知识库主闭环：狗狗档案、当前狗狗切换、健康记录（饮食和饮水已拆分）、护理记录（梳毛和美容已拆分）、洗澡、遛狗计时、日历回看、历史记录编辑删除、狗狗头像、爱宠时光、护理计划、护理覆盖统计、提醒列表、通知中心和本地 SQLite 参考手册都已接入本地 Repository。头像照片选择后支持原生裁切；爱宠时光只引用系统相册媒体，不复制原件。
...
| 数据库 | 内容 | 特性 |
|---|---|---|
| `user.sqlite` | 狗狗档案、健康记录、护理与活动、爱宠时光、相册引用、就诊、处方原文、提醒、提醒日志 | 私有、可写、可备份、可彻底删除 |
...
| 图片与文件 | 头像使用 image_cropper + file_selector + path_provider；爱宠时光使用 iOS PhotoKit / Android Photo Picker 引用与 100 MB 缩略图缓存 |
...
目标：以随笔、可选 Emoji 心情和用户主动选择的系统相册媒体，组成可以编辑的成长时间线。

```mermaid
flowchart TD
    A["用户选择多张照片或一个视频"] --> B["保存系统相册引用和媒体元数据"]
    B --> C["填写日期、随笔和可选 Emoji"]
    C --> D["确认后写入本地时间线"]
    D --> E["按日期倒序展示并支持编辑删除"]
```
...
- 每篇允许不限数量照片或一个不限时长视频，两种媒体不混排。
- App 不复制原图/视频，只保存 PhotoKit 标识或 Android 持久 URI；系统相册删除原件后保留文字、日期和 Emoji。
- 图片每屏最多展示两张并支持横向分页；视频只在时间线加载封面，点击后通过系统媒体句柄播放。
- 心情贴纸为一个可选 Unicode Emoji，不生成图片文件，也不参与健康判断。
- 缩略图缓存最高 100 MB，可在设置页查看和清除，不进入正式数据备份。
- 用户可以修改日期、随笔、Emoji、媒体顺序或重新关联失效媒体；删除日志不会删除系统相册原件。

当前模型：`MemoryEntry(id, petId, occurredAt, note, moodEmoji, createdAt, updatedAt)` 与 `MemoryMediaRef(id, entryId, kind, platformRef, position, width, height, durationMs, capturedAt)`。
...
- [x] 爱宠时光首版采用用户手动选择系统相册媒体，不扫描整个相册。

## 18. 变更记录

| 日期 | 版本 | 内容 |
|---|---|---|
| 2026-07-20 | 1.0 | 接入爱宠时光时间线、随笔、Emoji 心情、无限照片或单视频相册引用、失效占位和 100 MB 缩略图缓存 |
