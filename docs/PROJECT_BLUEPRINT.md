...
截至 2026-06-24，App 已完成 P0 的记录管理最小闭环：狗狗档案、当前狗狗切换、健康记录（饮食和饮水已拆分）、护理记录（梳毛和美容已拆分）、洗澡、遛狗计时、日历回看、历史记录编辑删除和狗狗头像都已接入本地 Repository。照片选择后支持原生裁切（3:4 竖屏比例）。

仍处于设计或数据接口阶段的能力包括：提醒与通知、就诊和处方、通用附件、护理计划持久化、趋势提示、健康摘要与导出、完整备份恢复、本地知识库、OCR 和 GPS 路线。

当前推进原则是：先补齐已有 Repository 的 UI 能力和护理计划持久化，再进入提醒系统；趋势、知识库、OCR 和路线能力后置。
...
| 表 | 用途 |
|---|---|
| `knowledge_articles` | 标题、正文、适用犬只条件、地区和审核状态 |
...
- [ ] 建立 FTS5 本地知识搜索
- [ ] 建立规则解释、版本和测试框架
- [ ] 根据狗狗档案和历史生成护理候选，并实现主人开启、调整、忽略和关闭流程
