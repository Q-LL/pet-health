...
产品定位为：**宠物健康与护理履历 + 高可靠提醒 + 本地智能辅助 + 兽医协作导出**。
...
- 根据宠物档案、护理历史和生活事件提出可解释的护理计划候选，由主人选择开启
...
| 护理计划建议 | 根据物种、毛发、年龄、生活方式、护理历史和事件提出候选计划，由主人确认开启 | P0 |
...
护理记录与健康记录共享同一只宠物、时间线、日历、提醒、摘要、备份和导出能力。两者在界面上可以统一回顾，但在数据结构和算法解释中保留类型边界。

### 常见护理计划候选

| 护理类型 | 建议方式 | 重要边界 |
|---|---|---|
| 口腔家庭护理 | 对猫狗提供候选计划；主人选择是否开启及执行方式 | 使用宠物专用用品；异常、疼痛或牙龈炎症应咨询兽医，家庭护理不能替代专业检查 |
| 梳毛 | 根据物种、毛长、毛型、季节和既往打结情况提出频率候选 | 不使用所有宠物通用的固定频率 |
| 洗澡 | 优先根据生活方式、皮肤情况、兽医建议和宠物自身历史间隔提出候选 | 不把“越勤越好”作为规则，不对皮肤问题自行给出洗护治疗方案 |
| 指甲检查/修剪 | 建议定期检查；修剪频率由磨损速度和历史间隔决定 | 主人不熟悉或宠物抗拒时建议由专业人员处理 |
...
| 用品与环境清洁 | 食盆、水具、寝具、猫砂环境等可选家庭计划 | 与宠物身体护理分开标记，避免混入医疗记录 |
...
1. **档案驱动**：根据物种、毛型、年龄、生活方式和兽医备注筛选适用护理。
2. **历史驱动**：数据足够后，使用宠物自己的历史间隔生成下一次候选，不套用统一周期。
3. **事件驱动**：例如结束遛狗后询问是否记录足爪检查；新增专业美容后询问是否建立下次计划。
4. **专业计划驱动**：兽医或专业美容师给出的计划由主人原文录入，优先级高于 App 通用建议。
...
- 专业医嘱覆盖通用护理规则；涉及疼痛、异常分泌物、创口、持续瘙痒等情况时转为健康记录和就医准备提示。
...
| 优先级 | 范围 |
|---|---|
| P0 | 多宠物档案、结构化健康与护理记录、护理计划候选及用户确认、洗澡护理、基础遛狗计时与场所、就诊与处方原文、提醒与日志、时间线、日历、照片、本地搜索、低风险趋势提示、自动摘要、完整备份恢复 |
| P1 | 可选 GPS 遛狗路线、OCR、宠物时光机基础导入、体重等趋势图、慢病/术后模板、专业审核的红旗卡片、签名知识包更新、应用锁和加密备份 |
...
- [ ] 建立 FTS5 本地知识搜索
- [ ] 建立规则解释、版本和测试框架
- [ ] 根据宠物档案和历史生成护理候选，并实现主人开启、调整、忽略和关闭流程
- [ ] 将护理数据作为健康趋势的可解释上下文，不生成未经验证的因果结论
...
“宠物乏味度”不是确定事实或医学指标。界面应使用“路线丰富度”“路线重复程度”或“活动变化”，并说明这是基于有限轨迹的体验建议。
...
目标：读取用户授权照片的拍摄时间，把宠物历史照片整理成可以编辑的成长时间线。
...
- [ ] 宠物时光机首版只允许手动选照片，还是允许授权指定相册？

## 18. 变更记录

| 日期 | 版本 | 内容 |
|---|---|---|
| 2026-06-22 | 0.5 | 加入 GPS 与地图路线丰富度、用户自备 AI API、宠物时光机及对应隐私和实施边界 |
